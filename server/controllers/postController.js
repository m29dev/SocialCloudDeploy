...
const posts_create_post = async (req, res) => {
    try {
        ...
        if (!description) return res.status(403).json({ message: 'post cannot be created without description' })
...
const posts_feed_get = async (req, res) => {
    try {
        ...
        const posts = await Post.find({ userId: { $in: queryArray } }).sort({ createdAt: -1 })
...
const posts_update_put = async (req, res) => {
    try {
        ...
        if (!description) return res.status(403).json({ message: 'description cannot be empty' })

        const postBeforeEdit = await Post.findById({ _id: id })
        if (postBeforeEdit.userId !== userId) return res.status(403).json({
            message: 'only owner can edit posts data'
        })

        const resPost = await Post.findByIdAndUpdate({ _id: id }, { description }, { new: true })
...
const posts_like_put = async (req, res) => {
    try {
        ...
        const updatedPost = await Post.findByIdAndUpdate({ _id: id }, { likes: post.likes }, { new: true })
